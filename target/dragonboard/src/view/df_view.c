...
#define MIC_POWER_BAR_WITH_SHIFT         6
...
if(vertical_screen == 0)//横屏
{
#if !defined (_CONFIG_PLATFORM_ASTON)
    if(mic_activated){
...
#else
	     auto_window.desc.width  = (layer_config.width >> 2) + (layer_config.width >> 3);
#endif
}
else//竖屏
{
...
#else
	manual_window.desc.width  = (layer_config.width >> 2) + (layer_config.width >> 3);
#endif
}
else
{
...
static int switch_button_init(void)
{
    ...
    if (script_fetch("df_view", "switch_button_name", (int *)switch_button.name, 4)) {
...
#else
	x= (layer_config.width >> 1) + (layer_config.width >> 3);
	y=0;
    width  = (layer_config.width >> 2) + (layer_config.width >> 3);
...
{
	x= 0;
	y=(layer_config.height >> 1) + (layer_config.height >> 3);
    width  = (layer_config.width);
    height=(layer_config.height >> 2) + (layer_config.height >> 3);
}
...
if(vertical_screen == 0)
{
...
#else
	if(mic_activated){
	    wifi_window.desc.width  = (layer_config.width >> 2)- (layer_config.width>>MIC_POWER_BAR_WITH_SHIFT_TOTAL);
	    }
	else
	{
	...
	wifi_window.desc.posx   = (layer_config.width >> 2) + (layer_config.width >> 3);
	wifi_window.desc.posy   = 0;
	wifi_window.desc.height = layer_config.height;
#endif
}
...
{
	wifi_window.desc.posx   = 0;
	wifi_window.desc.posy   = (layer_config.height >> 2) + (layer_config.height >> 3);
	wifi_window.desc.height = (layer_config.height >> 2);
	wifi_window.desc.width  = (layer_config.width);
}
...
#else
	misc_window.desc.width  = (layer_config.width >> 2) + (layer_config.width >> 3);
#endif
    misc_window.desc.height = layer_config.height-misc_window.desc.posy;
}
else
{
	misc_window.desc.width  = (layer_config.width);
	misc_window.desc.height = (layer_config.height >> 2) + (layer_config.height >> 3)-misc_window.desc.posy;
}
...
#if !defined (_CONFIG_PLATFORM_ASTON)
   mic1_window.desc.posx   = (layer_config.width >> 1)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT_TOTAL);
   mic2_window.desc.posx   = (layer_config.width >> 1)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT_TOTAL);
#else
   mic1_window.desc.posx   = (layer_config.width >> 1) + (layer_config.width >> 3)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT_TOTAL);
   mic2_window.desc.posx   = (layer_config.width >> 1) + (layer_config.width >> 3)-(layer_config.width>>MIC_POWER_BAR_WITH_SHIFT_TOTAL);
#endif
...
static int df_config_init(void)
{
   ...
   if (script_fetch("mic", "mic1_used", &mic1_used, 1)) {
    ...
    if (script_fetch("tp", "activated", &tp_activated, 1)) {
        tp_activated=0;
        db_msg("core: can't fetch mic1_used, set to default: %d\n", tp_activated);
...
static int df_windows_init(void)
{
    ...
    if(layer_config.width > layer_config.height)
    	vertical_screen = 0;
    else
    	vertical_screen = 1;
   ...
   // mic_window_init();
    ...
    if(disp_output_type_t==DISP_OUTPUT_TYPE_LCD && tp_activated){
        tp_track_init();
    }
    ...
    *buf = 0;
    ...
    if (mouse_x == 0 && mouse_y == 0) {
        ...
        if (press && tp_activated) {
            ...
            if ((flag & 0x03) == 0x03 && tp_activated) {
