...
# insmod tp driver
tp_activated=`script_fetch "tp" "activated"`
if [ $tp_activated -eq 1 ]; then
...
key_activated=`script_fetch "key" "activated"`
if [ $key_activated -eq 1 ]; then
	key_module_path=`script_fetch "key" "module_path"`
	if [ -n "$key_module_path" ]; then
