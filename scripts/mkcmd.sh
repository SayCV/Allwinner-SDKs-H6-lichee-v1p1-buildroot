...
function prepare_toolchain()
{
	...
	if [ "x$1" = "xtoolchain" ] ; then
		tooldir=${LICHEE_OUT_DIR}/external-toolchain
	else
	    tooldir=${LICHEE_OUT_DIR}/external-toolchain_32
	fi
	...
	printf "\033[0;31;1mUSING TOOL: $LICHEE_TOOLCHAIN_PATH \033[0m\n"
}
...
function mkkernel()
{
	...
	if [ -n "`echo ${LICHEE_CHIP} | grep "sun5[0-9]i"`" ] && [ "x${LICHEE_ARCH}" = "xarm64" ] ; then
...
function mkrootfs()
{
	...
	tooldir_32=${LICHEE_OUT_DIR}/external-toolchain_32
